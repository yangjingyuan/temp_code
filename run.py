...
from android_world.agents import tool_gui_agent
...
# LLM configuration for tool_gui_agent.
_LLM_BASE_URL = flags.DEFINE_string(
    'llm_base_url',
    'http://localhost:8000/v1',
    'Base URL for the LLM API endpoint (OpenAI-compatible).',
)
...
_LLM_API_KEY = flags.DEFINE_string(
    'llm_api_key',
    'empty',
...
def _get_agent(
  ...
  # Tool GUI Agent.
  elif _AGENT_NAME.value == 'tool_gui_agent':
    agent = tool_gui_agent.ToolGuiAgent(
        env,
        llm_base_url=_LLM_BASE_URL.value,
        model_name=_LLM_MODEL_NAME.value,
        api_key=_LLM_API_KEY.value,
    )
  ...
  if (
      agent.name in ['M3A', 'T3A', 'SeeAct', 'ToolGuiAgent']
      and family
